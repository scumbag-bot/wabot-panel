...
const { sendMessageWithHeal } = require('./senderkey_auto_heal.js');
        ...
        try {
            ...
            for (const msg of messages) {
                ...
                try {
                    ...
                    console.error(`❌ Failed to send to ${msg.phone}`, err);
                }
            }
        } catch (err) {
                ...
                if (!id_group) {
                    ...
                    if (exist_idgroup?.id || (exist_idgroup && exist_idgroup[0]?.id)) {
                        if(mentions){
                            ...
                            await sendMessageWithHeal(sock, exist_idgroup.id || exist_idgroup[0].id, pesankirim, mention_list, res);
                        }else{
                            await sendMessageWithHeal(sock, id_group, pesankirim, [], res);
                        }
                    } else {
                        res.status(500).json({
                            status: false,
